...
// JobEntryLinks holds the hypermedia links for a single job entry.
type JobEntryLinks struct {
	Self struct {
...
// JobAttributes holds the attributes of a single NetBackup job.
//
// Numeric fields mirror the NetBackup API types. AcceleratorOptimization and
// DedupRatio are ratios and are returned by the API as JSON floats (e.g. 0.0),
// so they are typed float64 to avoid unmarshal failures (see issue #17).
type JobAttributes struct {
	...
	AcceleratorOptimization    float64   `json:"acceleratorOptimization"`
...
// JobData represents a single job entry in the NetBackup /admin/jobs response.
type JobData struct {
	Links      JobEntryLinks `json:"links"`
