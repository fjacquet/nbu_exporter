...
<task type="auto">
  ...
  1. The existing tests in concurrent_test.go (TestCollectorConcurrentCollect, etc.)
     already verify thread safety of the collector. With parallel collection,
     these tests become even more important.

  2. Run full test suite with race detector to catch any races from parallel execution:

     ```bash
     go test -race ./...
     ```

  3. Verify the parallel pattern works with build:
