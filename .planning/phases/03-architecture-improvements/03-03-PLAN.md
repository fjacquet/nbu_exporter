...
2. Update FetchJobDetails to work with typed aggregation:
   - Change signature to accept typed accumulators
   - Return typed values for aggregation

3. Update FetchAllJobs to use typed collections:
...
2. Update collectStorageMetrics:
```go
...
4. Update expose methods to use Labels() directly - THIS IS THE KEY CHANGE:
```go
...
5. Update exposeMetrics to pass typed slices:
```go
...
6. Remove strings.Split imports if no longer needed
  </action>
  <verify>
- `go build ./internal/exporter/...` compiles
