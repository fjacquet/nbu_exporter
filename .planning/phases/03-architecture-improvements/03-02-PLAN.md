...
3. Update NbuClient struct to use TracerWrapper instead of trace.Tracer:
```go
...
4. Update NewNbuClient signature to accept options:
```go
...
5. Remove the global tracer provider access block:
```go
// REMOVE this block:
...
6. Update all uses of `c.tracer` to use `c.tracing.StartSpan()` or `c.tracing.Tracer()`:
...
7. Update methods that check tracer for nil (recordHTTPAttributes, recordError, injectTraceContext):
...
8. Update NewNbuClientWithVersionDetection to pass options through:
```go
...
3. Update NewNbuCollector signature:
```go
...
4. Remove global tracer access:
```go
// REMOVE: tracer := otel.Tracer("nbu-exporter")
```

5. Update createScrapeSpan to use TracerWrapper:
```go
...
6. Remove nil-check in createScrapeSpan - TracerWrapper guarantees valid span

7. Update Collect method to remove span nil-checks:
```go
...
8. Update other methods (recordFetchError, updateScrapeSpan, etc.) to remove nil-checks
  </action>
  <verify>
...
2. Add import for trace package if needed:
```go
import "go.opentelemetry.io/otel/trace"
```

3. If telemetry.Manager doesn't expose TracerProvider(), this will need to be added to telemetry/manager.go:
```go
...
4. Remove the otel.GetTracerProvider() call from main.go if present
  </action>
  <verify>
- `go build ./...` compiles
...
2. Add tests for options pattern:
