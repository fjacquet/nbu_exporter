...
2. Update the client initialization in NewNbuClient() to add retry configuration:
```go
client := resty.New().
...
3. Add import for "net/http" if not already present.

4. Add a debug log when retry is triggered (optional, helps with troubleshooting):
...
2. After creating the resty client in NewNbuClient(), configure the underlying HTTP transport:
```go
...
3. Update the client creation to remove the SetTLSClientConfig call (now in Transport):
```go
client := resty.New().
    SetTimeout(defaultTimeout).
    SetRetryCount(retryCount).
...
2. Add test for connection pool configuration:
```go
...
3. Add test for TLS configuration in transport:
```go
...
4. Add imports if needed: "crypto/tls", "net/http", "time"
  </action>
  <verify>
