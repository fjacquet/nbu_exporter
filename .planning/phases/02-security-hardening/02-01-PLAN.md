...
must_haves:
  truths:
    - "API key is not visible in error messages or logs (uses MaskAPIKey) - memory dump protection impractical in Go per research"
...
1. Call `validateTLSConfig()` from `Validate()` after `validateNBUServerConfig()`:
...
1. Add import for "os" package if not present.

2. Add tests in Config_test.go:
...
1. Update the security warning log level from Warn to Error (more visible):
...
1. Add test in client_test.go to verify TLS configuration:
